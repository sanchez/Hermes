...
function parse_bullet(lines, template, buffer) {
    var indentNum = -4;
    while (global.lookupPair["Bullet"].test(lines.peek())) {
        var res = global.lookupPair["Bullet"].exec(lines.get());
        while (res[1].length != indentNum) {
            if (res[1].length > indentNum) {
                buffer += "<ul>";
                indentNum += 4;
            } else if (res[1].length < indentNum) {
                buffer += "</ul>";
                indentNum -= 4;
            }
        }
        buffer += "<li>";
        console.log("Bullet: " + res[2]);
        var newBulletLine = {
            lines: [res[2]],
            pos: 0,
            peek: function() {
        ...
        buffer = template.text(newBulletLine, buffer, false);
        buffer += "</li>";
    }
    while (indentNum >= -4) {
        buffer += "</ul>";
...
function parse_num(lines, template, buffer) {
    var indentNum = -4;
    while (global.lookupPair["Numbered"].test(lines.peek())) {
        var res = global.lookupPair["Numbered"].exec(lines.get());
        while (res[1].length != indentNum) {
            if (res[1].length > indentNum) {
                buffer += "<ol>";
                indentNum += 4;
